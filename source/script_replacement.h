...
extern u64 time_slow_effect_hash;
extern Vector3f time_slow_effect_vector;
...
int get_command_flag_cat_replace(u64 module_accessor, int category) {
    ...
    if (category == 0) {
        ...
        if(ControlModule::check_button_on_trriger(module_accessor, CONTROL_PAD_BUTTON_APPEAL_S_L)){
            SlowModule::set_whole(module_accessor, 5, 0);
            EffectModule::req_on_joint(module_accessor, time_slow_effect_hash, hash40("waist"), &time_slow_effect_vector, &time_slow_effect_vector, 1.0, &time_slow_effect_vector, &time_slow_effect_vector, false, 0, 0, 0);
            EffectModule::set_rate_last(module_accessor, 2.0);
        }
        else if(ControlModule::check_button_on_trriger(module_accessor, CONTROL_PAD_BUTTON_APPEAL_S_R)){
